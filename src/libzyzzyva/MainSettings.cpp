...
//
// Copyright 2015-2021 Twilight Century Computing.
...
const QString SETTINGS_SHOW_WILDCARD_MATCHES = "wordlist_show_wildcard matches";
const QString SETTINGS_SHOW_PROBABILITY_ORDER = "wordlist_show_probability_order";
const QString SETTINGS_SHOW_PLAYABILITY_ORDER = "wordlist_show_playability_order";
const QString SETTINGS_SHOW_HOOKS = "wordlist_show_hooks";
const QString SETTINGS_SHOW_HOOK_PARENTS = "wordlist_show_hook_parents";
const QString SETTINGS_USE_HOOK_PARENT_HYPHENS = "wordlist_use_hook_parent_hyphens";
...
const QString SETTINGS_QUIZ_REQUIRE_LEXICON_SYMBOLS = "quiz_require_lexicon_symbols";
const QString SETTINGS_QUIZ_AUTO_CHECK = "quiz_auto_check";
const QString SETTINGS_QUIZ_AUTO_ADVANCE = "quiz_auto_advance";
const QString SETTINGS_QUIZ_AUTO_END_AFTER_INCORRECT = "quiz_auto_end_after_incorrect";
const QString SETTINGS_QUIZ_MARK_MISSED_AFTER_INCORRECT = "quiz_mark_missed_after_incorrect";
const QString SETTINGS_QUIZ_MARK_MISSED_AFTER_TIMER_EXPIRES = "quiz_mark_missed_after_timer_expires";
...
const QString SETTINGS_QUIZ_TIMEOUT_DISABLE_INPUT_MSECS = "quiz_timeout_disable_input_msecs";
...
const QString DEFAULT_DEFAULT_LEXICON = Defs::LEXICON_CSW24;
...
const QString DEFAULT_LEXICON_STYLES = QString(
                                           "%1 and not %2: symbol +")
                                           .arg(Defs::LEXICON_CSW24)
                                           .arg(Defs::LEXICON_CUSTOM);
...
void MainSettings::readSettings()
{
    ...
    instance->programVersion = settings.value(SETTINGS_PROGRAM_VERSION).toString();
    ...
    instance->defaultLexicon = settings.value(SETTINGS_DEFAULT_LEXICON).toString();
    if (instance->defaultLexicon.isEmpty())
    {
        instance->defaultLexicon = settings.value(SETTINGS_IMPORT_LEXICON).toString();
        if (instance->defaultLexicon.isEmpty())
    ...
    instance->autoImportLexicons = settings.value(SETTINGS_IMPORT_LEXICONS).toStringList();
    ...
    instance->autoImportFile = settings.value(SETTINGS_IMPORT_FILE).toString();

    instance->displayWelcome = settings.value(SETTINGS_DISPLAY_WELCOME,
                                              DEFAULT_DISPLAY_WELCOME)
                                   .toBool();

    instance->confirmExit = settings.value(SETTINGS_CONFIRM_EXIT,
    ...
    instance->useTileTheme = settings.value(SETTINGS_USE_TILE_THEME,
                                            DEFAULT_USE_TILE_THEME)
                                 .toBool();
    instance->tileTheme = settings.value(SETTINGS_TILE_THEME, DEFAULT_TILE_THEME).toString();

    instance->searchSelectInput = settings.value(SETTINGS_SEARCH_SELECT_INPUT,
                                                 DEFAULT_SEARCH_SELECT_INPUT)
                                      .toBool();

    instance->quizLetterOrder = settings.value(SETTINGS_QUIZ_LETTER_ORDER,
                                               DEFAULT_QUIZ_LETTER_ORDER)
                                    .toString();
    ...
    instance->quizUseFlashcardMode = settings.value(SETTINGS_QUIZ_USE_FLASHCARD_MODE,
                                                    DEFAULT_QUIZ_USE_FLASHCARD_MODE)
                                         .toBool();
    instance->quizShowNumResponses = settings.value(SETTINGS_QUIZ_SHOW_NUM_RESPONSES,
                                                    DEFAULT_QUIZ_SHOW_NUM_RESPONSES)
                                         .toBool();
    instance->quizShowQuestionStats = settings.value(SETTINGS_QUIZ_SHOW_QUESTION_STATS,
                                                     DEFAULT_QUIZ_SHOW_QUESTION_STATS)
                                          .toBool();
    instance->quizRequireLexiconSymbols = settings.value(SETTINGS_QUIZ_REQUIRE_LEXICON_SYMBOLS,
                                                         DEFAULT_QUIZ_REQUIRE_LEXICON_SYMBOLS)
                                              .toBool();
    instance->quizAutoCheck = settings.value(SETTINGS_QUIZ_AUTO_CHECK,
                                             DEFAULT_QUIZ_AUTO_CHECK)
                                  .toBool();
    instance->quizAutoAdvance = settings.value(SETTINGS_QUIZ_AUTO_ADVANCE,
                                               DEFAULT_QUIZ_AUTO_ADVANCE)
                                    .toBool();
    instance->quizAutoEndAfterIncorrect = settings.value(SETTINGS_QUIZ_AUTO_END_AFTER_INCORRECT,
                                                         DEFAULT_QUIZ_AUTO_END_AFTER_INCORRECT)
                                              .toBool();
    instance->quizMarkMissedAfterIncorrect = settings.value(SETTINGS_QUIZ_MARK_MISSED_AFTER_INCORRECT,
                                                            DEFAULT_QUIZ_MARK_MISSED_AFTER_INCORRECT)
                                                 .toBool();
    instance->quizMarkMissedAfterTimerExpires = settings.value(SETTINGS_QUIZ_MARK_MISSED_AFTER_TIMER_EXPIRES,
                                                               DEFAULT_QUIZ_MARK_MISSED_AFTER_TIMER_EXPIRES)
                                                    .toBool();
    instance->quizCycleAnswers = settings.value(SETTINGS_QUIZ_CYCLE_ANSWERS,
                                                DEFAULT_QUIZ_CYCLE_ANSWERS)
                                     .toBool();
    instance->quizCycleAnswersPeriodMillisecs = settings.value(SETTINGS_QUIZ_CYCLE_ANSWERS_PERIOD_MSECS,
                                                               DEFAULT_QUIZ_CYCLE_ANSWERS_PERIOD_MSECS)
                                                    .toInt();
    instance->quizTimeoutDisableInput = settings.value(SETTINGS_QUIZ_TIMEOUT_DISABLE_INPUT,
                                                       DEFAULT_QUIZ_TIMEOUT_DISABLE_INPUT)
                                            .toBool();
    instance->quizTimeoutDisableInputMillisecs = settings.value(SETTINGS_QUIZ_TIMEOUT_DISABLE_INPUT_MSECS,
                                                                DEFAULT_QUIZ_TIMEOUT_DISABLE_INPUT_MSECS)
                                                     .toInt();

    instance->quizRecordStats = settings.value(SETTINGS_QUIZ_RECORD_STATS,
                                               DEFAULT_QUIZ_RECORD_STATS)
                                    .toBool();

    instance->probabilityNumBlanks = settings.value(SETTINGS_PROBABILITY_NUM_BLANKS,
                                                    DEFAULT_PROBABILITY_NUM_BLANKS)
                                         .toInt();

    instance->setCardboxScheduleList(
        settings.value(SETTINGS_CARDBOX_SCHEDULES,
                       DEFAULT_CARDBOX_SCHEDULES)
            .toString());

    instance->setCardboxWindowList(
        settings.value(SETTINGS_CARDBOX_WINDOWS,
                       DEFAULT_CARDBOX_WINDOWS)
            .toString());

    instance->judgeSaveLog = settings.value(SETTINGS_JUDGE_SAVE_LOG,
                                            DEFAULT_JUDGE_SAVE_LOG)
                                 .toBool();

    instance->mainFont = settings.value(SETTINGS_FONT_MAIN).toString();
    instance->wordListFont = settings.value(SETTINGS_FONT_WORD_LISTS).toString();
    instance->quizLabelFont = settings.value(SETTINGS_FONT_QUIZ_LABEL).toString();
    instance->wordInputFont = settings.value(SETTINGS_FONT_WORD_INPUT).toString();
    instance->definitionFont = settings.value(SETTINGS_FONT_DEFINITIONS).toString();
    instance->printingFont = settings.value(SETTINGS_FONT_PRINTING).toString();

    instance->wordListSortByLength = settings.value(SETTINGS_SORT_BY_LENGTH,
                                                    DEFAULT_SORT_BY_LENGTH)
                                         .toBool();
    instance->wordListGroupByAnagrams = settings.value(SETTINGS_GROUP_BY_ALPHAGRAMS,
                                                       DEFAULT_GROUP_BY_ANAGRAMS)
                                            .toBool();
    instance->wordListShowWildcardMatches = settings.value(SETTINGS_SHOW_WILDCARD_MATCHES,
                                                           DEFAULT_SHOW_WILDCARD_MATCHES)
                                                .toBool();
    instance->wordListShowProbabilityOrder = settings.value(SETTINGS_SHOW_PROBABILITY_ORDER,
                                                            DEFAULT_SHOW_PROBABILITY_ORDER)
                                                 .toBool();
    instance->wordListShowPlayabilityOrder = settings.value(SETTINGS_SHOW_PLAYABILITY_ORDER,
                                                            DEFAULT_SHOW_PLAYABILITY_ORDER)
                                                 .toBool();
    instance->wordListShowHooks = settings.value(SETTINGS_SHOW_HOOKS, DEFAULT_SHOW_HOOKS).toBool();
    instance->wordListShowHookParents = settings.value(SETTINGS_SHOW_HOOK_PARENTS,
                                                       DEFAULT_SHOW_HOOK_PARENTS)
                                            .toBool();
    instance->wordListUseHookParentHyphens = settings.value(SETTINGS_USE_HOOK_PARENT_HYPHENS,
                                                            DEFAULT_USE_HOOK_PARENT_HYPHENS)
                                                 .toBool();
    instance->wordListShowDefinitions = settings.value(SETTINGS_SHOW_DEFINITIONS,
                                                       DEFAULT_SHOW_DEFINITIONS)
                                            .toBool();
    instance->wordListShowOneSensePerLine = settings.value(SETTINGS_SHOW_ONE_SENSE_PER_LINE,
                                                           DEFAULT_SHOW_ONE_SENSE_PER_LINE)
                                                .toBool();
    instance->wordListLowerCaseWildcards = settings.value(SETTINGS_LOWER_CASE_WILDCARDS,
                                                          DEFAULT_LOWER_CASE_WILDCARDS)
                                               .toBool();
    instance->wordListUseLexiconStyles = settings.value(SETTINGS_USE_LEXICON_STYLES,
                                                        DEFAULT_USE_LEXICON_STYLES)
                                             .toBool();

    instance->setWordListLexiconStyles(
        settings.value(SETTINGS_LEXICON_STYLES,
                       DEFAULT_LEXICON_STYLES)
            .toString());

    instance->letterDistribution = settings.value(SETTINGS_LETTER_DISTRIBUTION,
...
//! all groups
//---------------------------------------------------------------------------
void MainSettings::restoreDefaults(const QString &group)
{
...
//---------------------------------------------------------------------------
void MainSettings::setCardboxScheduleList(const QString &str)
{
...
//! @param str the string
//---------------------------------------------------------------------------
void MainSettings::setCardboxWindowList(const QString &str)
{
...
//! @param str the string
//---------------------------------------------------------------------------
void MainSettings::setWordListLexiconStyles(const QString &str)
{
